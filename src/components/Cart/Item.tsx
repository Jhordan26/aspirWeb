...
import { deleteCartItem, getFoodById, updateCartItemQty } from "../../utils/functions";
import { useStateValue } from "../../context/StateProvider";

const CartItem = ({ item }: { item: cartItem }) => {
  const [{ foodItems, cartItems }, dispatch] = useStateValue();
  const { id, qty } = item;
  const foodItem = getFoodById(foodItems,id);
    ...
    <div className="w-full p-1 px-2 rounded-lg bg-cartItem hover:shadow-md flex items-center justify-between gap-2 cursor-pointer ">
      <div className=" flex items-center  gap-2">
        ...
        <div className="flex flex-col gap-0 ">
          <p className="text-base text-gray-50">{foodItem?.nombre}</p>
          <p className="text-sm block text-gray-300 font-semibold">
            <span className="text-xs text-red-600">₵</span> {foodItem?.plan_precio}
          </p>
        </div>
      </div>
